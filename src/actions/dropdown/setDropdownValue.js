...
export function setDropdownValue(props, userID, e) {
    let classlist = e.target.classList,
        eventsToSet,
        usersToSet,
        pickedUsers = [...createNewStore().getState().pickedUsers],
        userToAdd = [...createNewStore().getState().users].filter(el => el._id === userID)[0];
    
    if (props !== undefined) {
        eventsToSet = [...createNewStore().getState().events].filter(el => el.eventID !== props.eventID);
    }   
    ...
    if (props !== undefined) {
        if (usersToSet !== undefined) {
            ...
            createNewStore().dispatch(updateEventData([...eventsToSet, getEventData(props.eventID, usersToSet)]));    
        }
        ...
        if (pickedUsers !== undefined) {
            ...
            createNewStore().dispatch(setPickedUsers(pickedUsers));    
        }
    }
